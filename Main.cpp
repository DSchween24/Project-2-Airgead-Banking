...
InterestCalc InitialDisplay(InterestCalc inputData) {
	// print inital data and receive input
	double deposit;
	double monthly;
	double interest;
	int years;
...
int main() {
	char ch; // Detect a keystroke
	
	do {
		//Display the initial view and receive input
		InterestCalc inputData;
		inputData = InitialDisplay(inputData);
		//Get keystroke to continue
		ch = _getch();

		// Display the report without the monthly deposits
	...
	while (ch != NULL); //Take care of input exceptions
}
